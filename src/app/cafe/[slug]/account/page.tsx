...
export default async function CustomerAccountPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="cafe-dark min-h-screen bg-forest-900 text-cream-50 bg-forest-glow bg-fixed pb-20">
      ...
      <div className="bg-forest-gradient text-cream-50 px-4 pt-4 pb-6 relative overflow-hidden">
        <div className="absolute inset-0 bg-pattern opacity-10" />
        <div className="container max-w-3xl relative">
          <Link
            href={`/cafe/${params.slug}`}
            className="text-forest-300 inline-flex items-center gap-1 text-sm hover:text-gold"
          >
            <ArrowLeft className="h-4 w-4" /> Back to {cafe.name}
          </Link>
          <div className="mt-3 flex items-center gap-3">
            <span className="grid h-12 w-12 place-items-center rounded-2xl bg-gold-gradient text-forest-950 font-display text-xl font-bold">
              {initial}
            </span>
            <div className="flex-1 min-w-0">
              ...
              <div className="text-forest-300 text-xs font-mono">{session.phone}</div>
            </div>
            <a
              href={`/cafe/${params.slug}/account/logout`}
              className="text-xs text-forest-300 hover:text-gold shrink-0 inline-flex items-center gap-1"
              title="Sign out"
            >
      ...
      <div className="container max-w-3xl -mt-4 space-y-4">
        {/* Big loyalty card */}
        {loyaltyOn ? (
          <div className="rounded-2xl bg-gold-gradient text-forest-950 p-5 shadow-coffee relative overflow-hidden">
            <div className="absolute -top-10 -right-10 h-32 w-32 rounded-full bg-white/20 blur-2xl" />
            <div className="absolute -bottom-10 -left-10 h-32 w-32 rounded-full bg-white/10 blur-2xl" />
            <div className="relative flex items-start gap-4">
              <div className="grid h-14 w-14 place-items-center rounded-2xl bg-forest-950/10 backdrop-blur shrink-0">
                <Award className="h-7 w-7" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-[11px] uppercase tracking-wider opacity-70">Loyalty balance</div>
                <div className="font-display text-4xl font-bold leading-none mt-0.5">
                  {points.toLocaleString()}
                  <span className="text-base font-semibold opacity-70 ml-1">pts</span>
                </div>
          ...
          <div className="rounded-2xl border border-dashed border-gold/30 bg-forest-800 p-4 text-sm text-forest-300 flex items-center gap-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div className="cafe-card p-3">
            <div className="text-[11px] text-forest-300">Orders here</div>
            <div className="font-display text-2xl font-bold text-cream-50">{totalOrders}</div>
          </div>
          <div className="cafe-card p-3">
            <div className="text-[11px] text-forest-300">Total spent</div>
            <div className="font-display text-2xl font-bold text-gold-light">{formatCurrency(totalSpent, cafe.currency)}</div>
          </div>
        </div>
        ...
        <Link
          href={`/cafe/${params.slug}`}
          className="block rounded-2xl bg-gold-gradient text-forest-950 p-4 shadow-coffee text-center hover:-translate-y-0.5 transition"
        >
        ...
        {activeOrders.length > 0 && (
          <div>
            <h2 className="font-display text-lg font-bold text-cream-50 mb-2 flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-gold" /> In progress
            </h2>
            <div className="space-y-2">
              {activeOrders.map((o) => (
                <div key={o.id} className="cafe-card p-5">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="font-bold text-cream-50">#{o.orderNumber}</div>
                      <div className="text-xs text-forest-300 truncate">
                    ...
                    <span className="cafe-pill-gold text-[10px] shrink-0">{ORDER_STATUS_LABELS[o.status] ?? o.status}</span>
                  </div>
                  <div className="mt-3 pt-3 border-t border-white/10 flex items-center justify-between gap-2">
                    <div className="font-bold text-gold-light">{formatCurrency(o.totalAmount, cafe.currency)}</div>
                    <div className="flex items-center gap-3">
                      <Link
                        href={`/order/${o.id}`}
                        className="text-xs font-semibold text-gold hover:text-gold-light inline-flex items-center gap-1"
                      >
        ...
        {loyaltyOn && (loyalty?.transactions?.length ?? 0) > 0 && (
          <div>
            <h2 className="font-display text-lg font-bold text-cream-50 mb-2 flex items-center gap-2">
              <Award className="h-4 w-4 text-gold" /> Points activity
            </h2>
            <div className="cafe-card divide-y divide-white/10">
              {loyalty!.transactions.map((t) => (
                <div key={t.id} className="px-4 py-2.5 flex items-center justify-between gap-3 text-sm">
                  <div className="min-w-0">
                    ...
                    <div className="text-[10px] text-forest-300">{formatDate(t.createdAt, { dateStyle: 'medium', timeStyle: 'short' })}</div>
                  </div>
                  <div className={`font-bold shrink-0 ${t.type === 'REDEEM' ? 'text-rose-400' : 'text-emerald-400'}`}>
        ...
        <div>
          <h2 className="font-display text-lg font-bold text-cream-50 mb-2 flex items-center gap-2">
            <Receipt className="h-4 w-4 text-gold" /> Past orders
          </h2>
          {pastOrders.length === 0 && activeOrders.length === 0 ? (
            ...
            <div className="cafe-card p-5 text-center text-forest-300 py-6 text-sm">
            ...
            <div className="space-y-2">
              {pastOrders.map((o) => (
                <Link key={o.id} href={`/order/${o.id}`} className="cafe-card p-5 block hover:-translate-y-0.5 hover:border-gold/40 transition">
                  <div className="flex items-center justify-between">
                    <div className="min-w-0">
                      <div className="font-semibold text-cream-50 truncate">{o.items.slice(0, 2).map((i) => i.name).join(', ') || `#${o.orderNumber}`}</div>
                    ...
                    <div className="text-right shrink-0 ml-2">
                      <div className="font-bold text-gold-light">{formatCurrency(o.totalAmount, cafe.currency)}</div>
                      <span className="cafe-pill-gold text-[10px]">{ORDER_STATUS_LABELS[o.status] ?? o.status}</span>
                    </div>
                  </div>
                </Link>
              ))}
